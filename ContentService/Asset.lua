...
Properties:
	love:Font font - Source.
	String align - Alignment.
				   "topleft",	"topcenter",	"topright"
				   "middleleft","middlecenter",	"middleright"
				   "bottomleft","bottomcenter",	"bottomright"
	Color color - Text color.
	Number+nil wrap - Maximum width when drawing a text. No limit if set to nil.
]]
local FontAsset = Instance:class("FontAsset",4)({
	...
	color = Color:new(),
	wrap = nil,
	draw = function(self,text,x,y,angle,sx,sy)
		...
		if self.wrap then
			local _,text = font:getWrap(text,self.wrap)
			direction = #text*direction
			for i,v in pairs(text) do
				love.graphics.printf(v,pos.x,pos.y+direction*height,screen,align,angle,sx)
				direction = direction+1
			end
		else love.graphics.printf(text,pos.x,pos.y+height*direction,screen,align,angle,sy)
		end
	end
})